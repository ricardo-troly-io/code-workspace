...
def print_member_orders comp, session, sheet
	export_sheet = RakeHelper::init_google_worksheet "Member Template Export", sheet
	import_sheet = RakeHelper::init_google_worksheet "Member Template Import", sheet
...
def print_company_products comp, session, sheet
	...
	comp.products.where(:archived_at => nil, :variety => [0, 2]).order(name: :asc).each_with_index do |prod, pdx|
		data = {
			'ID' => prod.id,
			'Name' => prod.name,
		...
		product_sheet = RakeHelper::set_wsheet_row(product_sheet, pdx, data, 0)
		puts "Saved #{prod.name}"
	end
	product_sheet.save
end

company = RakeHelper::company_lookup
